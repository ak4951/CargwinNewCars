...
def verify_competitor_prices():
    # Get auth token
    auth_response = requests.post('https://dealfinder-246.preview.emergentagent.com/api/auth/magic', 
                                 json={'email': 'admin@cargwin.com'})
    token = auth_response.json()['debug_token']

    verify_response = requests.post('https://dealfinder-246.preview.emergentagent.com/api/auth/verify',
                                   json={'token': token})
    access_token = verify_response.json()['access_token']

    # Get the RX350 Premium lot
    headers = {'Authorization': f'Bearer {access_token}'}
    response = requests.get('https://dealfinder-246.preview.emergentagent.com/api/admin/lots/690bbe50e52e2fafa277660e', 
                           headers=headers)
