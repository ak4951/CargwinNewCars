...
def calculate_lease_pro(
    ...
    # In California, monthly payments are taxed
    tax_amount = base_payment_before_tax * tax_rate
    monthly_payment_with_tax = base_payment_before_tax + tax_amount

    # 9. Upfront Tax (California Rule: Tax on Cap Cost Reduction)
    # CCR includes Down Payment and Taxable Incentives
    # We also assume doc_fee is taxable if not capitalized
    taxable_upfront = request.down_payment + total_incentives_applied
    upfront_tax = taxable_upfront * tax_rate

    # 10. Estimated Drive-Off
    if request.drive_off_mode == "zero":
        # Zero drive-off: customer pays only first month + gov fees upfront
        # Note: In a true zero drive-off, upfront tax and fees are capitalized
        # Here we follow the simple structure for now
        estimated_drive_off = monthly_payment_with_tax + doc_fee + registration_fee
    else:
        # Standard: down payment + first month + fees + upfront tax
        estimated_drive_off = request.down_payment + monthly_payment_with_tax + doc_fee + registration_fee + upfront_tax
