...
Живой сайт `https://cargwin-newcar.emergent.host/` **не проверен**: сетевая политика
этой среды отдаёт `403` на CONNECT к этому хосту. DNS резолвится (Cloudflare,
`104.18.14.241`, `104.18.15.241`) — домен существует, недоступность создана песочницей,
а не сайтом.
...
## 4. Расхождение доменов

| Место | Домен |
|---|---|
| `index.html`, `og:url` и `twitter:url` | `cargwin-newcar.emergent.host` |
| `public/robots.txt`, три директивы `Sitemap:` | `hunter.lease` |
| `App.js:82`, поле `url` в схеме | `hunter.lease` |
| `ai_schema_generator.py`, `url` и `logo` | `hunter.lease` |

Какой бы домен ни был боевым, часть ссылок ведёт не туда. Если это
`cargwin-newcar.emergent.host` — все три карты сайта в `robots.txt` указывают на чужой
хост и не будут прочитаны.

Плюс: тега `rel="canonical"` в `index.html` нет вообще. Для SPA с параметрами в URL это
прямой путь к дублям.
...
| 5 | Свести домены к одному и добавить `rel="canonical"` | средний | низкие |
